...
namespace ResportesParaDeveloper.Services
{
    public class DirectoryAnalyzerService
    {
        ...
        public AnalysisResult AnalyzeDirectory(string rootPath, FilterOptions filterOptions = null)
        {
            ...
            result.RootDirectory = AnalyzeDirectoryRecursive(rootPath, extensionDict, filterOptions);
            result.Extensions = extensionDict.Values.OrderBy(e => e.Extension).ToList();
            result.TotalFiles = CountFiles(result.RootDirectory);
            result.TotalDirectories = CountDirectories(result.RootDirectory);
        ...
        private DirectoryAnalysisInfo AnalyzeDirectoryRecursive(string path, Dictionary<string, ExtensionInfo> extensionDict, FilterOptions filterOptions)
        {
            ...
            try
            {
                ...
                foreach (var subDirPath in directories)
                {
                    try
                    {
                        ...
                        var subDir = AnalyzeDirectoryRecursive(subDirPath, extensionDict, filterOptions);
